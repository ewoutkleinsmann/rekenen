...
/**
 * Stat-based gating for track segments. These rules are authoritative for
 * win/loss so the game balance (and tests) stay deterministic regardless of
 * physics noise. Ported from the original 2D race so behaviour is preserved.
 */
  ...
  switch (segment.type) {
    case "curve":
      ...
      if (velocity > maxCurveSpeed(stats, segment.radius) * 1.1) {
        if (stats.handling < segment.minGrip) {
          return {
            ok: false,
